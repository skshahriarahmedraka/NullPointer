...
func (H *DatabaseCollections) SearchQuesListQueryWithMetadata(c *fiber.Ctx) error {


    fmt.Println("🚀 ~ file: SearchQuesListQueryWithMetadata.go ~ line 30 ~ ifc.Params ~ c.Params(\"search\") : ", c.Params("search"))
    fmt.Println("🚀 ~ file: SearchQuesListQueryWithMetadata.go ~ line 31 ~ ifc.Params ~ c.Query(\"order\") : ", c.Query("order"))
    fmt.Println("🚀 ~ file: SearchQuesListQueryWithMetadata.go ~ line 31 ~ ifc.Params ~ c.Query(\"stop\") : ", c.Query("stop"))
    fmt.Println("🚀 ~ file: SearchQuesListQueryWithMetadata.go ~ line 31 ~ ifc.Params ~ c.Query(\"start\") : ", c.Query("start"))
    fmt.Println("🚀 ~ file: SearchQuesListQueryWithMetadata.go ~ line 31 ~ ifc.Params ~ c.Query(\"type\") : ", c.Query("type"))
	...
	if c.Params("search")!="" && c.Query("type") != "" && c.Query("start") != "" && c.Query("stop") != "" && c.Query("order") != "" {
		...
		case "time":
			...
			filter = bson.D{
				...
				{Key:"QuesTitle",Value: bson.M{"$regex": SearchQuery, "$options": "i"}},
			}
			limit, _ = strconv.Atoi(c.Query("stop"))
			opts.SetLimit(int64(limit))
			cursor, err = H.MongoQuestionCol.Find(ctx, filter, opts)
			logs.Error("search Error while finding ques data  time", err)

			if err != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
					"message": "Internal Server Error while fetching search ques data time",
				})
			}
		...
		case "views":
			...
			opts.SetLimit(int64(limit))
			cursor, err = H.MongoQuestionCol.Find(ctx, filter, opts)
			logs.Error("search Error while finding ques data views", err)

			if err != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
					"message": "Internal Server Error while fetching search ques data views",
				})
			}
		...
		case "unanswered":
			...
			filter = bson.D{
				{Key: "QuesAnsAccepted", Value: bson.D{{Key: "$eq", Value: ""}}},
				{Key:"QuesTitle",Value: bson.M{"$regex": SearchQuery, "$options": "i"}},
			}
			limit, _ = strconv.Atoi(c.Query("stop"))
			opts.SetLimit(int64(limit))
			cursor, err = H.MongoQuestionCol.Find(ctx, filter, opts)
			logs.Error("search Error while finding ques data unanswered", err)

			if err != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
					"message": "Internal Server Error while fetching search  ques data unanswered",
				})
			}
		case "votes":
			// not working
    ...
    //   "Answers": null
		...
		default:
			...
			if err != nil {
				return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
					"message": "Internal Server Error while fetching search ques data votes",
				})
			}
