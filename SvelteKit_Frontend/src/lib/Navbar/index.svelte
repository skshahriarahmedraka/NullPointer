...
import WhiteDot from './svgs/WhiteDot.svelte';
	...
	import Nullpointer from '$lib/icons/null pointer.svelte';
	 ...
	 let UserData=  {
        ...
        "Badges":{ "Reputation":13452543,"Gold":999,"Silver":888,"Bronze":777},
...
{#if UserdataLoading}
	...
	>
		...
		<!-- Company logo -->
		<button
			class="flex h-full w-fit flex-row items-center justify-center rounded-lg px-3 hover:bg-slate-800 "
			on:click={() => {
				goto('/');
			}}
		>
		...
		<!-- spaces -->
		<button class="rounded-lg hover:bg-slate-800" on:click={() => goto('/s')}>
			<Spaces />
		</button>
		<!-- question for you -->
		<button
		...
		>
			...
			<div class="max-w-36   flex h-fit  min-w-min flex-row items-center justify-center">
				<p class="mx-1 font-semibold text-white">{ShortenNumber(UserData['Badges']['Reputation'])}</p>
						...
						>
							...
							{:else if i[0] === 'reward'}
								...
								<div class=" flex w-80 flex-col gap-1">
									<div class="StackTextColor flex flex-row  font-mono ">
										{#if Number(i[2]) < 0}
											<div class=" mx-1 font-semibold text-red-500 text-xs ?">{i[2]}</div>
										{:else}
		...
		<div class="   ">
			...
			{#if NavbarOptsOpen}
				<div
					class=" z-30 fixed float-left  -ml-[340px] mt-2 flex max-h-screen min-h-fit w-96  flex-col overflow-scroll rounded-lg  border-2 border-gray-600 bg-[#2d2d2d]  text-gray-200"
				>
					<div class="flex flex-row mt-3 ml-2 mb-1">
						<img
							src={UserData['UserImage']}
							alt=""
						...
						<div class="flex flex-col justify-center items-center">
							...
							<p class=" font-semibold">{ShortenNumber(UserData['Badges']["Reputation"])} Reputation <WhiteDot/> {ShortenNumber(UserData["Follower"])} Follower </p>
						</div>
					</div>
					<!-- <button
						class="flex h-fit w-full  flex-row border-0 border-b-[1px] border-gray-700 bg-[#2d2d2d]  py-2 px-4 text-[#3196e3] transition-all duration-150  ease-linear  hover:bg-[#404245] "
					...
					<button class=" text-xl h-fit w-full  border-0 border-b-[1px] border-gray-700 bg-[#2d2d2d]  py-2 px-4 text-[#3196e3] transition-all duration-150  ease-linear  hover:bg-[#404245]  ">
						My followers 
					</button>
					<button class=" text-xl h-fit w-full  border-0 border-b-[1px] border-gray-700 bg-[#2d2d2d]  py-2 px-4 text-[#3196e3] transition-all duration-150  ease-linear  hover:bg-[#404245]  ">
					...
					<button class=" text-xl h-fit w-full  border-0 border-b-[1px] border-gray-700 bg-[#2d2d2d]  py-2 px-4 text-red-500 transition-all duration-150  ease-linear  hover:bg-[#404245]  ">
						Logout 
					</button>
