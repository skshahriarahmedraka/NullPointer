<script lang="ts">
	...
	import { fetchPostBlog, fetchUserFlairData } from '$lib/store/fetch';
	...
	let Loading = false;
	let BlogWriterData: UserFlairDataType;
	onMount(async ()=> {
	
	if (parseInt($UserData.UserID) != 0) {
		BlogWriterData = await fetchUserFlairData($UserData.UserID);
	...
	let ErrorMsg = {
		BlogTitle: [false, 'BlogTitle is not Valid'],
		BlogTags: [false, 'BlogTags are empty'],
		BlogDescription: [false, 'BlogDescription is too short']
	};
	function ValidBlogTitle(s:string): boolean{
		if (s.length < 5) {
			return false;
		} else {
			return true
	...
	function ValidBlogDescription(s:string): boolean{
		if (s.length < 15) {
			return false;
		} else {
			return true
	...
	async function OnSubmitBlog() {
		...
		if (!ErrorMsg.BlogTitle[0] && !ErrorMsg.BlogTags[0] && !ErrorMsg.BlogDescription[0]) {
	...
	let ThisBlogImage:HTMLImageElement ;
	let ThisImageInput:HTMLInputElement ;
	const onBlogImageSelected = (event: Event) => {
		const inputElement = event.target as HTMLInputElement;
		file = inputElement.files?.[0] ?? null;
		handleSubmit();
	};
	...
	const handleSubmit=async () =>{
		...
		try {
			...
			ResObj = await response.json();
			BlogData.Image = ResObj.ImageUrl;
			console.log("🚀 ~ file: index.svelte:668 ~ handleSubmit ~ BlogData.Image:", BlogData.Image)
...
{#if Loading}
	<div class=" mt-2 max-h-full min-h-screen w-[1400px] bg-transparent flex flex-col gap-5">
		...
		<div class="flex h-[360px] w-full flex-row">
			...
			<div class="flex h-full w-1/2 flex-col justify-start gap-2">
				...
				<div class="  text-left text-lg font-semibold text-gray-300   ">Blog Title :</div>
				
				<input
					bind:value={BlogData.Title}
					type="text"
					class="  mt-3 h-12 w-[95%] rounded-lg border-2 border-[#24262b] bg-[#303338] p-2  text-lg font-medium text-[#98999e]  outline-none  focus:border-sky-500 active:border-gray-800 "
				/>
				...
				<div class="  flex h-10 w-full flex-row gap-2">
					{#if BlogWriterData.UserImage != ""}

					<img
					src={BlogWriterData.UserImage}
						alt=""
						class=" h-10 w-10 rounded-xl object-cover"
					/>
					{:else}
						<UserAnonymous class="h-10 w-10 rounded-xl " />
					{/if}
					<div class="flex flex-col">
						<p class=" font-raleway text-sm font-semibold text-blue-600">{BlogWriterData.UserName}</p>
						<p class=" font-light text-gray-500">{BlogWriterData.UserTitle}</p>
					</div>
				</div>
			</div>
			<!-- blog image -->
			<!-- <div class=" h-full w-1/2">
				<img
					src={'https://149351115.v2.pressablecdn.com/wp-content/uploads/2023/05/050123-Stack-Overflow-AI-isnt-the-app-2-1200x630.jpg'}
					alt=""
					class=" h-full w-full"
				/>
			</div> -->
			<!-- <p class="text-xl font-semibold">Small Poster Image :</p> -->
	<div class=" relative  flex  h-full w-1/2 flex-row justify-center ">
		{#if BlogData.Image != ''}
			<!-- banner image -->
			...
			<img
				src={ BlogData.Image}
				bind:this={ThisBlogImage}
				on:error={ ()=>{ThisBlogImage.src=BlogData.Image}}
				alt="ProfileImage"
		...
		<CameraPlus class=" h-full w-full rounded-xl  border-2 stroke-slate-400 border-slate-200 transition-all ease-linear duration-150 hover:stroke-slate-500 hover:border-slate-500 stroke-[1px] hover:cursor-pointer" />
			...
			<button class=" absolute   h-full   w-full self-end  " on:click={() => ThisImageInput.click()}>
				...
				<input
					...
					on:change={(e) => onBlogImageSelected(e)}
					type="file"
